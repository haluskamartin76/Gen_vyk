...
st.set_page_config(page_title="Generátor Výkazu", layout="centered")

st.title("📄 Generátor pracovného výkazu")
st.write("Vyplňte údaje a upravte zmeny v tabuľke.")

# --- FORMULÁR ---
with st.container():
    meno = st.text_input("Meno a Priezvisko:", placeholder="napr. Martin Haluska")
    ...
    c3, c4 = st.columns(2)
    with c3:
        zmena_skupina = st.selectbox("Základný cyklus:", ["Zmena 1", "Zmena 2", "Zmena 3", "Zmena 4"])
    with c4:
...
st.divider()
st.subheader(f"Upresnenie dní pre {mesiac_meno}")
...
for d in range(1, dni_count + 1):
    ...
    with col2: d_val = st.checkbox(" ", value=(vzor[pos] == "D"), key=f"d_{d}")
    with col3: n_val = st.checkbox(" ", value=(vzor[pos] == "N"), key=f"n_{d}")
    with col4: abs_val = st.text_input("Absencia", key=f"a_{d}", label_visibility="collapsed", placeholder="D/KZ/PN/L")
    with col5: 
        label = f"{DNI_SK[dt.weekday()]}"
...
if st.button("💾 VYGENEROVAŤ EXCEL VÝKAZ", type="primary", use_container_width=True):
    ...
    wb.save(output)
    st.download_button(label="📥 STIAHNUŤ VYGENEROVANÝ EXCEL", data=output.getvalue(), file_name=f"Vykaz_{meno.replace(' ','_')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
