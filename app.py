...
def ziskaj_sviatky(rok, mesiac):
    sviatky = {1: [1, 6], 5: [1, 8], 7: [5], 8: [29], 9: [1], 11: [1, 17], 12: [24, 25, 26]}
    velka_noc = {
...
aktualne_sviatky = ziskaj_sviatky(rok, mesiac_idx)
...
for d in range(1, dni_count + 1):
    ...
    # Určenie farby pozadia riadku v Streamlite
    row_style = ""
    if is_sviatok: row_style = "background-color: #e6f7ff;" # Svetlomodrá pre sviatok
    elif is_weekend: row_style = "background-color: #fffaf0;" # Jemná krémová pre víkend

    # Vytvorenie riadku pomocou kontajnera pre štýlovanie
    with st.container():
        ...
        with col5: 
            den_názov = f"{dt.strftime('%d.%m.')} ({DNI_SK[dt.weekday()]})"
            if is_sviatok: st.markdown(f"<span style='color:#0066cc; font-weight:bold;'>{den_názov} 🚩</span>", unsafe_allow_html=True)
            elif is_weekend: st.markdown(f"<span style='color:orange; font-weight:bold;'>{den_názov}</span>", unsafe_allow_html=True)
            else: st.write(den_názov)
...
if st.button("💾 VYGENEROVAŤ EXCEL VÝKAZ", type="primary", use_container_width=True):
    ...
    s_row = 11 + dni_count
    ws.merge_cells(f'A{s_row}:B{s_row}'); ws[f'A{s_row}'] = "Odpracované hodiny s p o l u:"; ws[f'A{s_row}'].alignment = Alignment(horizontal='left'); ws[f'A{s_row}'].font = f_bold
    for col_l in ["C", "D", "E", "H", "I", "J"]:
        ws[f'{col_l}{s_row}'] = f"=SUM({col_l}11:{col_l}{s_row-1})"; ws[f'{col_l}{s_row}'].font = f_bold; ws[f'{col_l}{s_row}'].border = border; ws[f'{col_l}{s_row}'].alignment = align_c
    ws.merge_cells(f'F{s_row}:G{s_row}'); ws[f'F{s_row}'] = f"=C{s_row}-I4"; ws[f'F{s_row}'].font = f_bold; ws[f'F{s_row}'].alignment = align_c; ws[f'F{s_row}'].border = border
    for c_idx in range(1, 11): ws.cell(s_row, c_idx).border = border
    
    sig_r = s_row + 3
    ws.merge_cells(f'B{sig_r}:D{sig_r}'); ws[f'B{sig_r}'] = "____________________________"; ws.merge_cells(f'G{sig_r}:I{sig_r}'); ws[f'G{sig_r}'] = "____________________________"
    ws.merge_cells(f'B{sig_r+1}:D{sig_r+1}'); ws[f'B{sig_r+1}'] = "podpis zamestnanca"; ws.merge_cells(f'G{sig_r+1}:I{sig_r+1}'); ws[f'G{sig_r+1}'] = "podpis vedúceho"
    for r_s in [sig_r, sig_r+1]:
        ws[f'B{r_s}'].alignment = align_c; ws[f'G{r_s}'].alignment = align_c

    widths = [8, 18, 10, 10, 10, 10, 10, 10, 12, 12]
