...
def ziskaj_sviatky(rok, mesiac):
    # Fixné sviatky
    sviatky = {
        1: [1, 6], 5: [1, 8], 7: [5], 8: [29], 9: [1, 15], 11: [1, 17], 12: [24, 25, 26]
    }
    # Výpočet Veľkej noci (približný pre 2024-2030)
    velka_noc = {
        2024: [(3, 29), (4, 1)], 2025: [(4, 18), (4, 21)], 2026: [(4, 3), (4, 6)],
        2027: [(3, 26), (3, 29)], 2028: [(4, 14), (4, 17)], 2029: [(3, 30), (4, 2)],
        2030: [(4, 19), (4, 22)]
    }
...
with st.sidebar:
    ...
    rok = st.selectbox("Rok:", list(range(2024, 2031)), index=2) # Predvolený 2026
...
st.subheader(f"Upresnenie dní pre {mesiac_meno} {rok} ({zmena_skupina})")
...
for d in range(1, dni_count + 1):
    ...
    with col2: d_val = st.checkbox(" ", value=(vzor[pos] == "D"), key=f"d_{d}_{st.session_state.reset_counter}")
    with col3: n_val = st.checkbox(" ", value=(vzor[pos] == "N"), key=f"n_{d}_{st.session_state.reset_counter}")
...
if st.button("💾 VYGENEROVAŤ EXCEL VÝKAZ", type="primary", use_container_width=True):
    ...
    # Riadok s textom hneď pod čiarou
    ...
    ws[f'B{sig_r+1}'].font = f_norm; ws[f'G{sig_r+1}'].font = f_norm
